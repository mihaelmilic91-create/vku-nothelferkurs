...
export const Route = createFileRoute("/anbieter/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Anbieter nicht gefunden" }, { name: "robots", content: "noindex" }] };
    }
...
function AnbieterDetail() {
  ...
  const ziel = [a.adresse, [a.plz, a.ort].filter(Boolean).join(" ")]
    .filter(Boolean)
    .join(", ") || a.name;
  ...
  return (
    <SiteShell>
      <PageHeader
        ...
        title={a.name}
        lead={[a.adresse, [a.plz, a.ort].filter(Boolean).join(" ")].filter(Boolean).join(", ")}
      />
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <span className="text-sm text-muted-foreground">Inkl. Rabattcode für deine Theorie-Vorbereitung.</span>
      </div>
      <GutscheinModal
        ...
        anbieterId={a.id}
        anbieterName={a.name}
        kontaktHref={a.termine_url ?? a.website_url ?? (a.kontakt_email ? `mailto:${a.kontakt_email}` : null)}
      />
      ...
      <div className="grid gap-6 pb-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-3xl bg-card p-6 shadow-[0_10px_30px_-22px_rgba(51,43,56,0.6)]">
          ...
          <div className="mt-3">
            ...
            <Zeile label="Preis">{a.preis_chf != null ? `CHF ${a.preis_chf}` : "auf Anfrage"}</Zeile>
            ...
            <Zeile label="Website">
              {a.website_url ? (
                <a href={a.website_url} className="text-coral" rel="noopener noreferrer" target="_blank">
                  {a.website_url}
                </a>
              ) : (
                "—"
            ...
            <Zeile label="Termine beim Anbieter">
              {a.termine_url ? (
                <a href={a.termine_url} className="text-coral" rel="noopener noreferrer" target="_blank">
                  Termine ansehen
                </a>
              ) : (
                "—"
